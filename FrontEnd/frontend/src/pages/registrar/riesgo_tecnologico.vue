        ...
        >
          <template v-slot:top>
            <div style="width: 100%" class="row justify-between">
              <div class="col-3 text-h6">Riesgo Tecnológico</div>
        ...
        <q-dialog v-model="data.cardCreate" persistent>
          <q-card class="my-card bg-primary" flat bordered>
            ...
            <form @submit.prevent.stop="onCreate">
              ...
              <q-checkbox
                v-model="data.sustancia"
                color="secondary"
                label="Materiales o sustancias peligrosas"
                true-value="si"
                false-value="no"
              ...
              <div class="q-pa-md" v-if="data.sustancia == 'si'">
                <q-table
                  title="Tabla de Materiales y Sustancias peligrosas"
                  :rows="data.rowsMateriales"
                ...
                <q-btn
                  no-caps
                  class="text-white bg-secondary q-pa-sm q-ma-sm"
                  @click="data.cardCreateMaterial = true"
                  >Agregar</q-btn
                >
                    ...
                    <form @submit.prevent.stop="onCreateMaterial">
                      <q-card-section class="row q-pa-sm">
                        <q-input
                          outlined
                          dense
                          v-model="data.tipoMaterial"
                          label="Nombre del material"
                          class="col-6 q-pa-sm text-black"
                          lazy-rules
                          :rules="alerts.inputRules"
                          ref="tipoMaterial"
                        />
                        ...
                        <q-input
                          outlined
                          dense
                          v-model="data.contencion"
                          label="Forma de contención"
                          class="col-12 q-pa-sm text-black"
                          lazy-rules
                          :rules="alerts.inputRules"
                          ref="contencion"
                        />
...
function editFields(params) {
  ...
  data.entidades.forEach((element) => {
    if (element.nombre == selected.value[0].entidad)
  ...
  if (selected.value[0].id_sustancia.length > 0) {
    ...
    for (
      let index = 0;
      index < selected.value[0].id_sustancia.length;
      index++
    ) {
    ...
    await api
      ...
      .then(function (response) {
        ...
        data.rowsMateriales.push({
          ...
          categoria:
            response.data.data.attributes.categoria.data[0].attributes
              .categoria,
          cantidad: response.data.data.attributes.cantidad,
          unidad:
            response.data.data.attributes.unidad.data[0].attributes.unidad,
...
async function getInstalacion(params) {
  ...
  for (let index = 1; index < 15; index++) {
    ...
    await api
      .get(
        `/instalacionespeligrosas/?populate=%2A&pagination[page]=${index}&pagination[pageSize]=100`,
        {
          headers: {
            Authorization: "Bearer " + auth.jwt,
          },
      ...
      .then(function (response) {
        ...
        for (let i = 0; i < response.data.data.length; i++) {
          ...
          for (
            let index = 0;
            index < response.data.data[i].attributes.sustancias.data.length;
            index++
          ) {
            element.push(
              response.data.data[i].attributes.sustancias.data[index].id
            );
          }
          let temp = "";
          //   const element = [];
          ...
          if (response.data.data[i].attributes.entidad.data != null) {
            if (response.data.data[i].attributes.anno == null)
              response.data.data[i].attributes.anno = "-";
            if (response.data.data[i].attributes.sustancias.data.length == 0)
              temp = "no";
            else temp = "si";
            data.rows.push({
              name: count,
              entidad:
                response.data.data[i].attributes.entidad.data.attributes
                  .entidad,
...
function onCreateMaterial() {
  ...
  if (
    categoria.value.hasError ||
    unidad.value.hasError ||
    tipoMaterial.value.hasError ||
    cantidad.value.hasError ||
    contencion.value.hasError ||
    alcance.value.hasError
  ) {
    alerts.alerts[0].message = "Rellene todo los campos obligatorios";
    $q.notify(alerts.alerts[0]);
    // form has error
